...
void key_vector_from_key_file(_Out_writes_all_(key_vector_length) uint8_t* key_vector, size_t key_vector_length, std::ifstream& key_file)
{
    ...
    CHECK_EXCEPTION(key_file.good(), "Error reading the key file.");
    key_file.read(reinterpret_cast<char*>(key_vector), key_vector_length);
    CHECK_EXCEPTION(key_file.good(), "Error reading the key file.");
}
