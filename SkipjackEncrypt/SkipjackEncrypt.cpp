...
void encrypt_file(_In_z_ const wchar_t* input_file_name, _In_z_ const wchar_t* output_file_name, _In_z_ const wchar_t* key_file_name)
{
    ...
    CrappyCrypto::Skipjack::encrypt_istream(input_file, output_file, key_vector);
}
...
int wmain(int argc, _In_reads_(argc) wchar_t** argv)
{
    ...
    try
    {
        ...
        if(argc == 4)
        {
            SkipjackEncrypt::encrypt_file(argv[arg_input_file_name], argv[arg_output_file_name], argv[arg_key_file_name]);
        }
        else
        {
